...
WalletState::WalletState(const std::string& wallet_name, const std::string& extended_private_key_str)
    : wallet_name(wallet_name) {
    
    // Create a new SecureMemory object to store the extended private key
    extended_private_key = std::make_unique<SecureMemory>(
        reinterpret_cast<const uint8_t*>(extended_private_key_str.data()),
        extended_private_key_str.size()
    );
...
bool WalletState::recover_wallet_state() {
    try {
        // Validate the extended private key
        if (!extended_private_key || extended_private_key->isEmpty()) {
            throw std::runtime_error("Extended private key is empty or invalid");
        }
        
        // Create a temporary string from the secure memory for Base58 decoding
        std::string temp_key(reinterpret_cast<const char*>(extended_private_key->data()), 
                            extended_private_key->size());
        
        // Decode the Base58Check-encoded extended private key
        auto decoded = wallet::Base58::decode(temp_key);
        ...
        // Clear the decoded data after deserializing
